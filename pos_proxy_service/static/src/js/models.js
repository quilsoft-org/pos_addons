...
models.PosModel = models.PosModel.extend({
    ...
    get_value_type: function(){
        ...
        var type = 83;
        if(localStorage['go_nc']==110)
            {
                type = 110;
            }
        if(client && localStorage['go_nc']!=110){
        ...
        if(client && localStorage['go_nc']==110){
            ...
            if (client.l10n_ar_afip_responsibility_type_id && !company_type_afip_monotributo){
                if(client.l10n_ar_afip_responsibility_type_id[1] == 'IVA Responsable Inscripto') type = 112; //Nota de crédito Factura A
                else if(client.l10n_ar_afip_responsibility_type_id[1] == 'Responsable Monotributo') type = 114;//Nota de crédito Factura C
                else if(client.l10n_ar_afip_responsibility_type_id[1] == 'Consumidor Final' || client.l10n_ar_afip_responsibility_type_id[1] == 'IVA Sujeto Exento') type = 113;//Nota de crédito Factura B
            }
            else if(company_type_afip_monotributo){
                type = 114;
            }
    ...
    get_values_items: function(){
       ...
       var type = this.get_value_type();
        ...
        for (var i = 0; i < order_lines.length; i++) {
            ...
            //console.log('cccc:',line.quantity);

            var item_vals = {
                'description' : line.product.display_name,
                'description_extra1' : '',
                'qty' : parseFloat(line.quantity.toString().replace("-", "")),
                'price' : price,
                'iva' : iva,
    ...
    get_values_paymentlines: function(){
        ...
        for (var i = 0; i < paymentlines.length; i++){
            ...
            console.log('montojm:',pay.amount);
            if(pay.amount<0){
                                elmontojm=pay.amount;
                                console.log('elmontojmjmjm',elmontojm);
                                var elmontoconvertidoapositivo=elmontojm.toString().replace("-", "");
                                pay.amount=parseFloat(elmontoconvertidoapositivo);
                               

                            }

             console.log('montojm2:',pay.amount);
